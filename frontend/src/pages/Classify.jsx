      ...
      <div className="grid gap-6 lg:grid-cols-[0.9fr_1.1fr]">
        <section className="panel p-6">
          ...
          <label
            ...
            className={`mt-5 flex min-h-72 cursor-pointer flex-col items-center justify-center rounded-md border border-dashed p-6 text-center transition ${
              loading
                ? "border-leaf bg-leaf/5"
                : "border-canopy/25 bg-white hover:border-leaf hover:bg-leaf/5"
            }`}
          >
            {preview ? (
              <img
          ...
          {loading ? (
            <div
              className="mt-4 flex items-center gap-3 rounded-md border border-leaf/20 bg-leaf/5 px-4 py-3 text-sm font-medium text-canopy"
              role="status"
              aria-live="polite"
            >
              <Loader2 className="animate-spin text-leaf" size={18} aria-hidden="true" />
              <span>Processing image with Model 1. Please wait.</span>
            </div>
          ) : null}
          <button
            type="button"
          ...
          >
            ...
            {loading ? "Processing image" : "Run classification"}
          </button>
